...
<head>
    ...
    <!--<link rel="stylesheet" href="${pageContext.request.contextPath}/admin/${pageContext.request.contextPath}/admin/${pageContext.request.contextPath}/admin/plugins/timepicker/bootstrap-timepicker.min.css">-->
            ...
            <div class="box box-primary">
                    ...
                    <div class="table-box">
                        ...
                        <div class="pull-left">
                            <div class="form-group form-inline">
                                <div class="btn-group">
                                    <button onclick="refresh()" type="button" class="btn btn-default" title="刷新"><i class="fa fa-refresh"></i> 刷新</button>
                                </div>
                            </div>
                        </div>
                        ...
                        <table id="dataList" class="table table-bordered table-striped table-hover dataTable">
                            ...
                            <tr>
                                ...
                                <%--</th>--%>
                                <th class="text-center sorting_asc">编号</th>
                                <th class="text-center sorting_desc sorting_desc_disabled">用户头像</th>
                                <th class="text-center sorting_asc sorting_asc_disabled">用户名</th>
                                <th class="text-center sorting_desc sorting_desc_disabled">QQ号</th>
                                <th class="text-center sorting sorting_desc_disabled">粉丝数</th>
                                <th class="text-center sorting sorting_desc_disabled">用户状态</th>
                            ...
                            <c:forEach items="${pageInfo.list}" var="user" varStatus="status">
                                <tr>
                                    ...
                                    <td class="text-center"><img src="${pageContext.request.contextPath}/picture/${user.picture}" width="50px" height="50px"></td>
                                    <td class="text-center">${user.username}</td>
                                    <td class="text-center">${user.qq}</td>
                                    <td class="text-center">${user.fans}</td>
                                    <td class="text-center">${user.statusStr}</td>
                                    <td class="text-center">
                                        <button type="button" class="btn bg-olive btn-xs" onclick="updateStatus('${user.id}','${user.status}')">修改</button>
                                    </td>
                                </tr>
                            </c:forEach>
                ...
                <div class="box-footer">
                    ...
                    <div class="box-tools pull-right">
                        <ul class="pagination">
                            <li>
                                <a href="${pageContext.request.contextPath}/user/showAll?pageNum=1&pageSize=${pageInfo.pageSize}" aria-label="Previous">首页</a>
                            </li>
                            <li><a href="${pageContext.request.contextPath}/user/showAll?pageNum=${pageInfo.prePage}&pageSize=${pageInfo.pageSize}">上一页</a></li>
                            <c:forEach begin="1" end="${pageInfo.pages}" var="pageNUm">
                            <li><a href="${pageContext.request.contextPath}/user/showAll?pageNum=${pageNUm}&pageSize=${pageInfo.pageSize}">${pageNUm}</a></li>
                            </c:forEach>
                            <li><a href="${pageContext.request.contextPath}/user/showAll?pageNum=${pageInfo.nextPage}&pageSize=${pageInfo.pageSize}">下一页</a></li>
                            <li>
                                <a href="${pageContext.request.contextPath}/user/showAll?pageNum=${pageInfo.pages}&pageSize=${pageInfo.pageSize}" aria-label="Next">尾页</a>
                            </li>
                        </ul>
                    </div>
...
<script>
    ...
    function updateStatus(id,status) {
        ...
        $.post(
            "${pageContext.request.contextPath}/user/updateStatus",
            "id="+id+"&status="+status,
            function () {
                location.reload();
            }
    ...
    function changePageSize() {
        ...
        //向服务器发送请求改变每页显示的条数
        location.href = "${pageContext.request.contextPath}/user/showAll?pageNum=1&pageSize="+pageSize;

        //保存pageSize
        localStorage.setItem("pageSize",pageSize);
    }
    //修改下拉框的值
    $("#changePageSize").val(localStorage.getItem("pageSize"))
